...
import {formatDynamicTimestamp, formatFilesize} from '../util/format.js'
...
function getGameTitle(baseInfo) {
  ...
  return `${playerAmount} player ${gameType} hosted by ${gameHost}`
}
 ...
 */
function getPlayerName(playerObj, apm = null) {
  return `${getRaceEmoji(playerObj.race)} ${playerObj.name}${apm ? ` (${apm} apm)` : ''}`
}
...
function getPlayerList(players) {
  return players.map(player => {
    return {
      ...
      eapm: player.eapm,
      race: player.race,
      nameFormatted: getPlayerName(player, player.apm)
    }
  })
}
 ...
 */
export function generateRichInfo(replay, baseInfo, bufferSize, useSpoilerMessages = true) {
  return {
    ...
    messages: formatChatToCodeBlock(baseInfo.messages, baseInfo.players, useSpoilerMessages),
    meta: {
